-- Add brand_manager to user_role enum
ALTER TYPE user_role ADD VALUE 'brand_manager';
...
ON public.brands 
FOR SELECT 
USING (get_user_role() = 'brand_manager'::user_role AND id = (SELECT brand_id FROM public.profiles WHERE user_id = auth.uid()));

-- Update stores RLS policies to include brand managers
CREATE POLICY "Brand managers can manage stores in their brand" 
ON public.stores 
FOR ALL 
USING (get_user_role() = 'brand_manager'::user_role AND brand_id = (SELECT brand_id FROM public.profiles WHERE user_id = auth.uid()));
...
USING (get_user_role() = 'brand_manager'::user_role AND store_id IN (
  SELECT stores.id FROM stores WHERE stores.brand_id = (SELECT brand_id FROM public.profiles WHERE user_id = auth.uid())
));
...
USING (get_user_role() = 'brand_manager'::user_role AND employee_id IN (
  ...
  WHERE s.brand_id = (SELECT brand_id FROM public.profiles WHERE user_id = auth.uid())
));
